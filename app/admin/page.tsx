...
export default function AdminDashboard() {
  ...
  const ADMIN_TOKEN = process.env.NEXT_PUBLIC_ADMIN_TOKEN || providedToken // fallback

  // Check authorization + load data
  useEffect(() => {
    const token = providedToken

    const authorized = !!token // For simplicity we trust the token presence (backend validates it)
    ...
    const fetchData = async () => {
      ...
      try {
        const [messagesRes, reviewsRes] = await Promise.all([
          fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5000'}/api/admin/messages?token=${token}`),
          fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5000'}/api/admin/reviews?token=${token}`),
        ])
    ...
    if (success) {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5000'}/api/admin/reviews?token=${token}`
        )
        if (res.ok) {
        ...
        <div className="mx-auto max-w-md">
          <div className="rounded-3xl border border-border bg-card p-8 text-center">
            ...
            <p className="mt-4 text-xs text-muted-foreground">
              Make sure <span className="font-mono">ADMIN_TOKEN</span> is set in your backend <span className="font-mono">.env</span>.
            </p>
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Manage contact messages and client reviews (MERN Backend)</p>
          </div>
          <Link
            href="/"
        ...
        {loading ? (
          <div className="flex h-64 items-center justify-center">
            <div className="text-muted-foreground">Loading data from backend...</div>
          </div>
        ) : (
          <>
